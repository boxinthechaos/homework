...
* 데비안
  * 이안머독에 의해 비영리 조직으로 데비안 프로젝트 설립
  * GNU의 공식적인 후원을 받고 있는 유일한 배포판
  * 정식판이 존재하는 것은 데비안 GNU/리눅스뿐임
  * 패키지 설치 및 업그레이드 과정이 단순 (패키지 매니저 apt)
...
* RHEL (Red Hat Enterprise Linux)
  * 레드햇의 기업용 유료(상용) 리눅스 배포판
  * 18 ~ 24개월에 한 번씩 새로운 버전이 공개되며 라이선스는 별도로 판매 X
  * 기술지원은 버전마다 출시 지점으로부터 7년 동안 제공

* 페도라 (Fedora)
  * 레드햇의 후원과 개발 공동체의 지원 아래 개발된 배포판
  * 6개월 간격으로 새로운 버전이 배포
  * 기술지원 기간은 각 버전마다 13개월
  * 소프트웨어 개발이 안정적으로 이루어지기 위해서는 새 버전으로 계속 교체되어야 하는 문제점

* CentOS
  * 업스트림 소스인 RHEL과 완벽히 호환되는 무료 기업용 컴퓨팅 운영체제
  * 레드햇의 기술 지원은 받지 않음

* 수세(SUSE)
  * 독일에서 출시된 배포판으로 유럽에서 인기
  * 정기적인 배포판이 존재하기보단 새로운 버전이 출시되면 언제든지 업데이트가 가능한 롤링 릴리즈 방식 사용
